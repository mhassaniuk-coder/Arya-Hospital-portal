      ...
      <div className="bg-gradient-to-r from-arya-600 to-arya-800 p-4 flex items-center justify-between text-white shadow-md z-10 shrink-0">
        ...
        <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full font-bold text-sm transition-all ${
                isLiveActive ? 'bg-red-500 hover:bg-red-600 text-white animate-pulse' : 'bg-white text-arya-700 hover:bg-arya-50'
            }`}
        >
          ...
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex max-w-[85%] md:max-w-[70%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`p-4 rounded-2xl shadow-sm text-sm md:text-base leading-relaxed ${
                    msg.text.includes('CONFIRMED') || msg.text.includes('EMERGENCY') || msg.text.startsWith('🚀') ? 'bg-transparent shadow-none p-0 w-full' :
